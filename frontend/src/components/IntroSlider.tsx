  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-transparent p-6 relative">
      ...
      <div className="w-full max-w-md mt-8 flex flex-col items-center">
        ...
        <Button
          onClick={handleNext}
          className={`group w-full text-lg py-6 rounded-xl shadow-lg hover:shadow-xl transition-all ${
            currentIndex === slides.length - 1 ? "bg-green-600 hover:bg-green-700 text-white" : ""
          }`}
        >
          {currentIndex === slides.length - 1 ? (
            <span className="flex items-center">
              Get Started <Check className="ml-2 h-5 w-5 stroke-[3]" />
            </span>
          ) : (
            <span className="flex items-center overflow-hidden relative">
              <span className="pr-6">Next</span>
              <motion.span
